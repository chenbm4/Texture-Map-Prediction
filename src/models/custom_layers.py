...
class minibatch_std_concat_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.averaging == 'all':
            ...
            if len(shape) == 4:
                vals = torch.mean(torch.mean(vals, 2, keepdim=True), 3, keepdim=True)             # mean(vals, axis=[2,3], keepdim=True)
        elif self.averaging == 'none':
            target_shape = [target_shape[0]] + [s for s in target_shape[1:]]
        elif self.averaging == 'gpool':
            if len(shape) == 4:
                vals = torch.mean(torch.mean(torch.mean(x, 2, keepdim=True), 3, keepdim=True), 0, keepdim=True)                   # mean(x, [0,2,3], keepdim=True)
            ...
            vals = torch.mean(vals, dim=0, keepdim=True).view(1, self.n, 1, 1)
