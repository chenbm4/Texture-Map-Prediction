...
from models import import_gan as net
...
from utils import tf_recorder as tensorboard
...
class trainer:
    def __init__(self, config):
        ...
        self.eps_drift = self.config.eps_drift
        self.smoothing = self.config.smoothing
        self.max_resl = self.config.max_resl
        ...
        self.E = UNet(n_channels=3, n_classes=3)
        self.G = net.Generator(self.config)
        self.D = net.Discriminator(self.config)
        ...
        if self.use_cuda:
            ...
            if self.config.n_gpu==1:
                ...
                gpus = []
                for i  in range(self.config.n_gpu):
                    gpus.append(i)
    ...
    def renew_everything(self):
        ...
        self.x = torch.FloatTensor(self.loader.batchsize, 3, 512, 512)
        self.x_tilde = torch.FloatTensor(self.loader.batchsize, 3, self.loader.imsize, self.loader.imsize)
        self.y = torch.FloatTensor(self.loader.batchsize, 3, self.loader.imsize, self.loader.imsize)
        ...
        if self.optimizer == 'adam':
            self.opt_e = Adam(filter(lambda p: p.requires_grad, self.E.parameters()), lr=self.lr, betas=betas, weight_decay=0.0)
    ...
    def feed_interpolated_input(self, y):
        if self.phase == 'gtrns' and floor(self.resl)>2 and floor(self.resl)<=self.max_resl:
            alpha = self.complete['gen']/100.0
            transform = transforms.Compose([
                transforms.ToPILImage(),
                transforms.Resize(size=int(pow(2, floor(self.resl) - 1)), interpolation=transforms.InterpolationMode.NEAREST),
                transforms.Resize(size=int(pow(2, floor(self.resl))), interpolation=transforms.InterpolationMode.NEAREST),
                transforms.ToTensor(),
            ])
            y_low = y.clone().add(1).mul(0.5)
            for i in range(y_low.size(0)):
                y_low[i] = transform(y_low[i]).mul(2).add(-1)
            y = torch.add(y.mul(alpha), y_low.mul(1-alpha)) # interpolated_y
    ...
    def train(self):
        for step in range(2, self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK, self.loader.batchsize)):
                ...
                self.stack += self.loader.batchsize
                ...
                # Get training batch
                x, y = self.loader.get_train_batch()
                self.x.data.copy_(x)
                self.y.data = self.feed_interpolated_input(y)

                # Zero the gradients on each optimizer
                self.opt_e.zero_grad()
                self.opt_g.zero_grad()
                self.opt_d.zero_grad()

                # Pass real images through the encoder to get latent vectors
                self.z = self.E(self.x)
                self.x_tilde = self.G(self.z.detach())

                # Discriminator update
                real_scores = self.D(self.y)
                fake_scores = self.D(self.x_tilde.detach())
                loss_d = discriminator_loss(self.D, self.y, self.x_tilde, self.device)
                loss_d.backward(retain_graph=True)
                self.opt_d.step()

                # Generator update
                loss_eg = generator_loss(self.D, self.x_tilde)
                loss_eg.backward()
                self.opt_g.step()
                self.opt_e.step()
                
                # logging.
                log_msg = ' [E:{0}][T:{1}][{2:6}/{3:6}]  errD: {4:.4f} | errG: {5:.4f} | [lr:{11:.5f}][cur:{6:.3f}][resl:{7:4}][{8}][{9:.1f}%][{10:.1f}%]'.format(self.epoch, self.globalTick, self.stack, len(self.loader.dataset), loss_d.item(), loss_eg.item(), self.resl, int(pow(2,floor(self.resl))), self.phase, self.complete['gen'], self.complete['dis'], self.lr)
                tqdm.write(log_msg)
                ...
                if self.globalIter%self.config.save_img_every == 0:
                    ...
                    generated_image_path = 'repo/save/resl_{}/{}_{}_G{}_D{}.jpg'.format(int(floor(self.resl)), int(self.globalIter/self.config.save_img_every), self.phase, self.complete['gen'], self.complete['dis'])
                    utils.save_image_single(x_test.data, generated_image_path)
                    
                    # Save the interpolated ground truth image
                    interpolated_image_path = 'repo/save/resl_{}/{}_{}_GT.jpg'.format(int(floor(self.resl)), int(self.globalIter/self.config.save_img_every), self.phase)
                    utils.save_image_single(self.y.data, interpolated_image_path)

                # tensorboard visualization.
                if self.use_tb:
                    # with torch.no_grad():
                    #     x_test = self.G(self.z_test)
                    self.tb.add_scalar('data/loss_eg', loss_eg.item(), self.globalIter)
                    ...
                    self.tb.add_image_grid('grid/x_intp', 4, utils.adjust_dyn_range(self.y.data.float(), [-1,1], [0,1]), self.globalIter)
                    # '''
    ...
    def get_state(self, target):
        if target == 'gen':
            ...
            state = {
                'resl' : self.resl,
                'state_dict' : self.E.module.state_dict(),
                'optimizer' : self.opt_e.state_dict(),
            }
            return state
    ...
    def snapshot(self, path):
        ...
        nenc = 'enc_R{}_T{}.pth.tar'.format(int(floor(self.resl)), self.globalTick)
        if self.globalTick%50==0:
            if self.phase == 'gstab' or self.phase =='dstab' or self.phase == 'final':
                ...
                if not os.path.exists(save_path):
                    ...
                    save_path = os.path.join(path, nenc)
                    torch.save(self.get_state('enc'), save_path)
                    print('[snapshot] model saved @ {}'.format(path))

if __name__ == '__main__':
    ## perform training.
    print('----------------- configuration -----------------')
    for k, v in vars(get_config()).items():
        print('  {}: {}'.format(k, v))
    print('-------------------------------------------------')
