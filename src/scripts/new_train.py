    ...
    def train(self):
        ...
        for step in range(2, self.max_resl+1+5):
            for iter in tqdm(range(0,(self.trns_tick*2+self.stab_tick*2)*self.TICK, self.loader.batchsize)):
                ...
                # apply noise to the real outputs if needed.
                if self.flag_add_noise:
                    self.y = self.add_noise(self.y)
                
                self.z = self.E(self.x)         # Get latent vector from encoder.
                self.x_tilde = self.G(self.z)   # Generate fake image from latent vector.

                # discriminate between real and fake.
                self.fx = self.D(self.y)                        # discriminator output for real image.
                self.fx_tilde = self.D(self.x_tilde.detach())   # discriminator output for fake image.
                
                loss_d = discriminator_loss(self.D, self.real_label, self.x_tilde, device=self.config.device)
                loss_d.backward()
                self.opt_d.step()

                # update generator.
                loss_eg = generator_loss(self.D, self.x_tilde, self.real_label)
                loss_eg.backward()
                self.opt_g.step()
                self.opt_e.step()
                ...
                if self.globalIter%self.config.save_img_every == 0:
                    ...
                    utils.save_image_grid(x_test.data, 'repo/save/grid/{}_{}_G{}_D{}.jpg'.format(int(self.globalIter/self.config.save_img_every), self.phase, self.complete['enc'], self.complete['gen'], self.complete['dis']))
                    utils.mkdir('repo/save/resl_{}'.format(int(floor(self.resl))))
                    utils.save_image_single(x_test.data, 'repo/save/resl_{}/{}_{}_G{}_D{}.jpg'.format(int(floor(self.resl)),int(self.globalIter/self.config.save_img_every), self.phase, self.complete['enc'], self.complete['gen'], self.complete['dis']))

                # tensorboard visualization.
                if self.use_tb:
                    # with torch.no_grad():
                    #     x_test = self.G(self.z_test)
                    self.tb.add_scalar('data/loss_eg', loss_eg[0].item(), self.globalIter)
